...
export default function Search() {
    const keySearch = useSearchParams().get('key');
    const [searchValue, setSearchValue] = useState(keySearch)
    const result = sectorNews.filter((item) => item.heading.toLowerCase().includes(searchValue?.toLowerCase() ?? ""))

    return <CenterContent>
        <h3 className="text-xl capitalize md:text-[35px] mt-8">Search results for values </h3>
            <input autoFocus type="text" id="default-input" value={searchValue ?? ""}  onChange={(e) => setSearchValue(e.target.value)} className=" mt-4 bg-gray-50 border outline-none border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#911320] focus:border-[#911320] block w-full p-2.5 " />
        <div
